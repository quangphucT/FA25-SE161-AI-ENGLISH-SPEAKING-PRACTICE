...
const Message = ({ type, text, timestamp }: MessageProps) => {
  ...
  return (
    <div className={`flex gap-3 mb-4 ${type === "user" ? "flex-row-reverse" : ""}`}>
      {/* Avatar */}
        ...
        <div className="bg-gradient-to-br from-blue-500 via-indigo-600 to-purple-600 rounded-2xl p-6 border border-blue-400 shadow-lg">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-xs font-bold px-3 py-1.5 rounded-full backdrop-blur-sm ${
              state === "speaking" ? "bg-white/30 text-white" : "bg-white/20 text-blue-100"
            }`}>
              {getStateText()}
            </span>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col h-full overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-gradient-to-b from-gray-50/50 to-white scroll-smooth">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-sm font-medium text-gray-600">Chưa có tin nhắn</p>
          ...
          <div className="px-6 py-3 border-t border-gray-200 bg-gray-50">
            <p className="text-xs text-gray-500 text-center">
              Tổng: <span className="font-semibold text-gray-700">{messages.length}</span> tin nhắn
            </p>
          </div>
        </div>
      </div>
